...
class SugangFetchService (
    ...
    fun fetchXls(
        ...
        // go to sugang site
        webClient.get().uri(initUrl).retrieve().toBodilessEntity().block()
        ...
        webClient.post().uri(ajaxUrl).header(HttpHeaders.REFERER, "https://sugang.snu.ac.kr/sugang/co/co010.action").contentType(
            MediaType.APPLICATION_FORM_URLENCODED).body(BodyInserters.fromFormData("openUpDeptCd", "").with("openDeptCd", "").with("srchOpenSchyy", year.toString()).with("srchOpenShtm", semCode)).retrieve().toBodilessEntity().block()

        // searching
        val search = baseSearchForm(year, semCode).toFormDataInserter()
        webClient.post().uri(searchUrl).header(HttpHeaders.REFERER, "https://sugang.snu.ac.kr/sugang/co/co010.action").contentType(
            MediaType.APPLICATION_FORM_URLENCODED).body(search).retrieve().toBodilessEntity().block()
    ...
    private fun baseExportForm(
        year: Int,
        semCode: String
    ): Map<String, String> = baseSearchForm(year, semCode).toMutableMap().apply{
        this["workType"] = "EX"
    }

    private fun Map<String, String>.toFormDataInserter()
